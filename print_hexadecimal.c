...
/*   Updated: 2017/12/08 12:55:56 by fpetras          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_0x_0X(uintmax_t nbr, t_struct *f, char letter)
{
	if (f->hash && letter == 'x' && nbr != 0)
		f->len += write(1, "0x", 2);
	else if (f->hash && letter == 'X' && nbr != 0)
...
static void	ft_print_hex2(uintmax_t nbr, t_struct *f, char letter)
{
	if (f->minus)
		ft_print_hex_left_align(nbr, f, letter);
	else
		ft_print_hex_right_align(nbr, f, letter);
}
